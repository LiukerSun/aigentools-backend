...
func GetAllModelsSimple() ([]models.AIModel, error) {
	...
	if err := database.DB.Select("id, name, description, status, url, price, created_at, updated_at").Where("status = ?", models.AIModelStatusOpen).Find(&modelsList).Error; err != nil {
		return nil, err
	}
